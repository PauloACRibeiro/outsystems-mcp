...
Step 4: when I tell you, run `claude mcp add -s user --transport http --client-id service_studio --callback-port 7890 outsystems https://mcp-test.datap-dev-us-east-1-01.dev-06.stamp.outsystemscloudrd.net/<my-tenant>/mcp` (substitute my actual tenant for `<my-tenant>`).
...
Step 5: when I tell you, set the URL `https://mcp-test.datap-dev-us-east-1-01.dev-06.stamp.outsystemscloudrd.net/<my-tenant>/mcp` in two files (read first, preserve every other entry):
